...
# PyTorch's torch.nn.RMSNorm has no native ONNX op; decompose into mean/pow/rsqrt.
...
# ── Pre-computed constants (remove runtime chess.square calls from forward) ──
RANK7_INDICES = [chess.square(f, 6) for f in range(8)]  # [48..55]
RANK8_INDICES = [chess.square(f, 7) for f in range(8)]  # [56..63]
...
class EncoderOnlyBlock(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        sa_out, _ = self.self_attn(query=x, key=x, value=x, attn_mask=attn_mask)
...
class MAIA3Model(nn.Module):
    def __init__(self, cfg):
        ...
        assert (self.cfg.use_gab + self.cfg.use_relative_bias + self.cfg.use_absolute_pe) >= 1
    ...
    def forward(self, tokens, self_elos, oppo_elos):
        ...
        self_elo_embs = self_elo_embs.unsqueeze(1).expand(-1, 64, -1)
        oppo_elo_embs = oppo_elo_embs.unsqueeze(1).expand(-1, 64, -1)
        ...
        rank8_indices = RANK8_INDICES

        rank8_features = sq_to[:, rank8_indices, :]
        ...
        for from_file in range(8):
            ...
            for to_file in range(8):
                ...
                base_score = scores_base[:, from_sq, to_sq]
                for piece_idx in range(4):
                    bias = promo_biases[:, to_file, piece_idx]
